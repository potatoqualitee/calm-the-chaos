...
async function checkAndInitialize() {
    try {
        ...
        if (!isExtensionEnabledOnUrl(window.location.href, ignoredDomains, disabledDomainGroups, filteringEnabled, enabledDomains)) {
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
    if (message.type === 'extensionReloaded' || message.type === 'updateKeywords') {
        // Re-check if extension should be enabled and reinitialize if needed
        checkAndInitialize();
    }
});
